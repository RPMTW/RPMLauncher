String MojangMetaAPI = "https://launchermeta.mojang.com/mc/game";
String FabricApi = "https://meta.fabricmc.net/v2";
String ForgeFilesAPI ="https://files.minecraftforge.net";
String ForgeFilesMainAPI ="https://files.minecraftforge.net/net/minecraftforge/forge";
String ForgeLatestVersionAPI ="$ForgeFilesMainAPI/promotions_slim.json";
String ForgeMavenUrl = "https://maven.minecraftforge.net";
String ForgeMavenMainUrl = "https://maven.minecraftforge.net/net/minecraftforge/forge";
String MojangJREAPI ="https://launchermeta.mojang.com/v1/products/java-runtime/2ec0cc96c44e5a76b9c8b7c39df7210883d12871/all.json";
String MojangAuthAPI = "https://authserver.mojang.com";
String CurseForgeModAPI = "https://addons-ecs.forgesvc.net/api/v2";
String ModrinthAPI = "https://api.modrinth.com/api/v1";
